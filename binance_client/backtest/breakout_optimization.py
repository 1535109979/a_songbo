...
def plt_account(symbol, n, period, roll_mean_period ,start_time,step_price):
...
if __name__ == '__main__':
    ...
    df_reslt = pd.DataFrame(columns=['symbol', 'value', 'sharpe', 'n', 'period', 'roll_mean_period'])
    ...
    for symbol in saved_symbol_list:
        print(symbol)
        for n in [100,200,300,400,500,600,700]:
            for period in [100,200,300,400,500,600,700]:
                for roll_mean_period in [100,200,300,400,500,600,700]:
                    plt_account(symbol, n, period, roll_mean_period, start_time, step_price)

        df_reslt.to_csv('df_reslt.csv', mode='a', index=False, header=False)
        df_reslt = df_reslt.sort_values(by='sharpe', ascending=False)
        print(df_reslt)
        df_reslt = df_reslt.iloc[0:0]
