# Generated by the gRPC Python protocol compiler plugin. DO NOT EDIT!
"""Client and server classes corresponding to protobuf-defined services."""
import grpc

import a_songbo.vn.proto.market_server_pb2 as market__server__pb2


class AsyncMarketServerStub(object):
    """Missing associated documentation comment in .proto file."""

    def __init__(self, channel):
        """Constructor.

        Args:
            channel: A grpc.Channel.
        """
        self.GetQuoteStream = channel.unary_stream(
                '/AsyncMarketServer/GetQuoteStream',
                request_serializer=market__server__pb2.Symbols.SerializeToString,
                response_deserializer=market__server__pb2.Quote.FromString,
                )
        self.AddSubscribe = channel.unary_unary(
                '/AsyncMarketServer/AddSubscribe',
                request_serializer=market__server__pb2.Symbols.SerializeToString,
                response_deserializer=market__server__pb2.FlagReply.FromString,
                )
        self.StopEngine = channel.unary_unary(
                '/AsyncMarketServer/StopEngine',
                request_serializer=market__server__pb2.FlagReply.SerializeToString,
                response_deserializer=market__server__pb2.FlagReply.FromString,
                )


class AsyncMarketServerServicer(object):
    """Missing associated documentation comment in .proto file."""

    def GetQuoteStream(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def AddSubscribe(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def StopEngine(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')


def add_AsyncMarketServerServicer_to_server(servicer, server):
    rpc_method_handlers = {
            'GetQuoteStream': grpc.unary_stream_rpc_method_handler(
                    servicer.GetQuoteStream,
                    request_deserializer=market__server__pb2.Symbols.FromString,
                    response_serializer=market__server__pb2.Quote.SerializeToString,
            ),
            'AddSubscribe': grpc.unary_unary_rpc_method_handler(
                    servicer.AddSubscribe,
                    request_deserializer=market__server__pb2.Symbols.FromString,
                    response_serializer=market__server__pb2.FlagReply.SerializeToString,
            ),
            'StopEngine': grpc.unary_unary_rpc_method_handler(
                    servicer.StopEngine,
                    request_deserializer=market__server__pb2.FlagReply.FromString,
                    response_serializer=market__server__pb2.FlagReply.SerializeToString,
            ),
    }
    generic_handler = grpc.method_handlers_generic_handler(
            'AsyncMarketServer', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))


 # This class is part of an EXPERIMENTAL API.
class AsyncMarketServer(object):
    """Missing associated documentation comment in .proto file."""

    @staticmethod
    def GetQuoteStream(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_stream(request, target, '/AsyncMarketServer/GetQuoteStream',
            market__server__pb2.Symbols.SerializeToString,
            market__server__pb2.Quote.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)

    @staticmethod
    def AddSubscribe(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/AsyncMarketServer/AddSubscribe',
            market__server__pb2.Symbols.SerializeToString,
            market__server__pb2.FlagReply.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)

    @staticmethod
    def StopEngine(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/AsyncMarketServer/StopEngine',
            market__server__pb2.FlagReply.SerializeToString,
            market__server__pb2.FlagReply.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)
