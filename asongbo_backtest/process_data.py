...
with sqlite3.connect('./database/future_min_data.db') as conn:
    df = pd.read_sql('select * from future_daily_data', conn)

df = df.sort_values(by='date')

df['return'] = df.groupby('variety')['close_price'].pct_change(periods=160)
...
plt.title(f'change position interval={interval} trading day, pct_change interval=60')
